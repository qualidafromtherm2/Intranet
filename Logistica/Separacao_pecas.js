...
// Exemplo de trecho de código para carregar itens do CSV
// e filtrar apenas os que tenham status "L1" (em vez de "T1").
// Verifique se o servidor Node.js está rodando e se o endpoint correto está sendo usado.
// Por exemplo, se o seu backend está na porta 5001, ajuste a URL no fetch:

async function loadTasksFromCSV() {
  try {
    // Use a URL correta do backend, por exemplo, 127.0.0.1:5001
    ...
    rows.forEach(row => {
      if (row.Pedido && row.produto && row.status && row.status.trim() === "L1") {
        addTaskToKanban(row);
      }
    });
...
function updateHeaderCounts() {
  ...
  if (h3Todo) {
    // Nome fixo "Separação de peças" seguido da contagem de itens
    h3Todo.textContent = `Separação de peças (${count})`;
  } else {
...
function updateProductCodesListbox() {
  ...
  doneCards.forEach(card => {
    ...
    // Supondo que o texto seja "Produto: FL180F40T"
