...
router.post('/', (req, res) => {
  ...
  let csvContent = "";
// NOVO CABEÇALHO: 11 colunas
const header = "OP,Pedido,produto,local,status,data,user,observação,data_previsao,descricao,caracteristica\n";
  ...
  dados.forEach(row => {
    ...
    csvContent += `${opCode},${row.pedido},${row.produto},${localVal},${statusVal},${dataVal},${userVal},${csvSafeField(observacao)},${row.data_previsao || ""},${row.descricao || ""},${csvSafeField(row.caracteristica || "")}\n`;
