/**
 * Combobox de Tipo de assunto (catálogo sac.tipo_assunto).
 * Mesmo padrão do Tipo de falha: opções cadastradas, × para excluir, "+ Inserir" para criar.
 * Usado em Nova OS e Editar OS → Atendimento.
 */
(function (global) {
  const API = '/api/sac/at/tipos-assunto';
  let _cache = null;
  let _cacheAt = 0;
  const CACHE_MS = 5000;
  ...
  function avisoCatalogo() {
    if (typeof global._atOnTipoAssuntoCatalogoChange === 'function') {
      try { global._atOnTipoAssuntoCatalogoChange(); } catch (_) { /* ignore */ }
    }
  }
  ...
  async function list(force) {
    ...
    if (!resp.ok || data.ok === false) throw new Error(data.error || 'Falha ao listar tipos de assunto.');
  ...
  function mount(el, opts = {}) {
    ...
    let aberto = false;
    let salvando = false;
    const emptyLabel = opts.emptyLabel || 'Selecione';
    ...
    livreWrap.innerHTML = `
      <input type="text" class="at-tipo-falha-livre" maxlength="120" placeholder="Novo tipo de assunto...">
    ...
    function syncTrigger() {
      trigger.textContent = value || emptyLabel;
    ...
    async function renderMenu() {
      ...
      const parts = [];
      parts.push(`
        <div class="at-tipo-falha-item${!value ? ' is-selected' : ''}" data-nome="">
          <span class="at-tipo-falha-nome">${esc(emptyLabel)}</span>
        </div>`);
      if (!tipos.length) {
