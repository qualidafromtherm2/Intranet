...
// ============================================================================
// GET /api/produtos/tem-entrada-nfe?codigo=09.MC.N.10106&codigo_produto=1072290
// Responde se o produto JÁ teve ao menos uma entrada com NF-e REAL — seja pelo
// fluxo normal de associação de NF-e (etiquetas de recebimento) ou pela NF-e
// sincronizada da Omie. O "Recebimento sem NF-e" (manual) NÃO conta: ele grava
// motivo_sem_nfe / omie_ent_codigo e numero_nfe tipo "SEM-NFE"/data, então é
// explicitamente excluído aqui.
// Usado pelo modal de ações do produto para esconder o botão "Recebimento sem
// NF-e" quando o produto já entrou via NF-e alguma vez.
// ============================================================================
router.get('/tem-entrada-nfe', async (req, res) => {
  try {
    const codigo = String(req.query?.codigo || '').trim();
    const codigoProduto = String(req.query?.codigo_produto || '').trim();
    if (!codigo && !codigoProduto) {
      return res.status(400).json({ ok: false, error: 'Informe ?codigo ou ?codigo_produto.' });
    }

    // Valores possíveis para casar o produto (SKU interno e/ou ID Omie).
    const idsProduto = [codigo, codigoProduto].map((v) => String(v || '').trim()).filter(Boolean);

    let temNfe = false;

    // Fonte A: NF-e real sincronizada da Omie (recebimentos_nfe_itens/omie).
    try {
      const sqlOmie = `
        SELECT 1
          FROM logistica.recebimentos_nfe_itens i
         ...
         WHERE (
                 ($1 <> '' AND i.n_id_produto::text = $1)
              OR ($2 <> '' AND TRIM(COALESCE(i.c_codigo_produto, '')) = $2)
               )
           AND COALESCE(r.c_cancelada, 'N') <> 'S'
         LIMIT 1
      `;
      const { rows } = await dbQuery(sqlOmie, [codigoProduto, codigo]);
      if (rows.length) temNfe = true;
    } catch (err) {
      console.error('[produtos/tem-entrada-nfe] fonte Omie falhou →', String(err));
    }

    // Fonte B: etiquetas de recebimento pelo fluxo normal de associação de NF-e
    // (exclui recebimento manual sem NF-e).
    if (!temNfe && idsProduto.length) {
      try {
        const sqlEtq = `
          SELECT 1
            FROM etiqueta."ETQ_recebimento" er
           WHERE TRIM(COALESCE(er.codigo_produto, '')) = ANY($1::text[])
             AND (er.motivo_sem_nfe IS NULL OR TRIM(er.motivo_sem_nfe) = '')
             AND er.omie_ent_codigo IS NULL
             AND UPPER(TRIM(COALESCE(er.numero_nfe, '')))    NOT LIKE 'SEM-NFE%'
             AND UPPER(TRIM(COALESCE(er.numero_pedido, ''))) NOT LIKE 'MANUAL%'
           LIMIT 1
        `;
        const { rows } = await dbQuery(sqlEtq, [idsProduto]);
        if (rows.length) temNfe = true;
      } catch (err) {
        console.error('[produtos/tem-entrada-nfe] fonte etiquetas falhou →', String(err));
      }
    }

    res.set('Cache-Control', 'no-store');
    return res.json({ ok: true, temNfe });
  } catch (err) {
    console.error('[produtos/tem-entrada-nfe] erro →', err);
    return res.status(500).json({ ok: false, error: 'Falha ao verificar entradas com NF-e.' });
  }
});
