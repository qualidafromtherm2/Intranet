...
function MeetingPanel({id,kind,eventDate,participants,onPrint}:{id:number;kind:'minutes'|'notes'|'attendance';eventDate:string;participants:string[];onPrint?:()=>void}) {
 ...
 const add=async()=>{ if(!text.trim()||!window.confirm('Confirmar gravação desta anotação?'))return; if(kind==='minutes') await calendarGateway.createMinute({reserva_id:id,tema:theme,conteudo:text},true); else if(kind==='notes') await calendarGateway.createNote({reserva_id:id,texto:text,anexo_url:null,anexo_nome:null,visivel_todos:shared},true); if(reminderDate) await calendarGateway.createNotice({data:reminderDate,texto, destinatarios:participants, apenas_mencionados:true, ata_id:kind==='minutes'?id:undefined},true); setText(''); setReminderDate(''); };
