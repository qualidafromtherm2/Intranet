...
vi.mock('./RequesterSeparationScreen', () => ({ RequesterSeparationScreen: () => <div>Solicitações autorizadas</div> }))
vi.mock('../../services/separationGateway', async (original) => ({ ...(await original<typeof import('../../services/separationGateway')>()), loadSeparationKanban: vi.fn(), loadSeparationOperatorContext: vi.fn(), loadSeparationItems: vi.fn(), loadSeparationOperationalData: vi.fn(), assumeSeparation: vi.fn(), cancelSeparation: vi.fn(), startSeparation: vi.fn(), deleteSeparation: vi.fn(), deleteSeparationItem: vi.fn() }))
...
beforeEach(() => {
  ...
  vi.mocked(gateway.loadSeparationOperationalData).mockResolvedValue({ stock: {}, addresses: {}, fifo: {} })
})
...
describe('SeparationWorkspace operational permissions', () => {
  it('starts in cart and exposes the collaborator destination scope', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Tela de separação' }))
  ...
  it('allows assuming another operator but hides item mutations until ownership changes', async () => {
    ...
    render(<SeparationWorkspace />); fireEvent.click(screen.getByRole('button', { name: 'Tela de separação' }))
