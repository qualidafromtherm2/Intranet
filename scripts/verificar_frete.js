...
async function main() {
  const [fontes, normalizados, municipios, status, ultimaCotacao, origem] = await Promise.all([
    ...
    pool.query('SELECT COUNT(*)::int AS total FROM frete.municipio'),
  ...
  console.log(JSON.stringify({
    ...
    municipios_ibge: municipios.rows[0].total,
